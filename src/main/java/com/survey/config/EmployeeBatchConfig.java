...
//        tokenizer.setNames("employeeId", "name", "email", "position", "status", "joinDate", "departmentId");
     ...
     // 🟡 CHANGE: departmentId to departmentName
        tokenizer.setNames("employeeId", "name", "email", "position", "status", "joinDate", "departmentName");
    ...
    // Step 3: Step - Let Spring auto-wire the existing @Component processor
    @Bean
    public Step employeeImportStep(JobRepository jobRepository,
                                   PlatformTransactionManager transactionManager,
                                   ItemReader<EmployeeRequestDTO> reader,
                                   ItemProcessor<EmployeeRequestDTO, Employee> processor, // Auto-wired from @Component
