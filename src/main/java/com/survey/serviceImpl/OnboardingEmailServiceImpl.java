...
public class OnboardingEmailServiceImpl implements OnboardingEmailService {
    ...
    public void sendWelcomeEmailsForBatch(Long employeeBatchLogId) {
        ...
        // CHECK if emails have already been sent for this batch
        OnboardingEmailLog existingLog = onboardingEmailLogRepository.findByEmployeeBatchLogId(employeeBatchLogId);
        if (existingLog != null && "COMPLETED".equals(existingLog.getStatus())) {
            log.info("Emails have already been sent for batch {}. Skipping...", employeeBatchLogId);
            return;
        ...
        try {
            ...
            // 🟡 CRITICAL FIX: Get employees ONLY from this specific batch
            List<String> batchEmails = parseBatchEmployeeEmails(employeeBatchLog.getBatchEmployeeEmails());
            List<Employee> batchEmployees = employeeRepository.findByEmailIn(batchEmails);
            
            onboardingEmailLog.setTotalEmployees(batchEmployees.size());
            onboardingEmailLogRepository.save(onboardingEmailLog);
            
            log.info("Processing {} employees from batch {} for onboarding welcome emails", 
                    batchEmployees.size(), employeeBatchLogId);
            ...
            if (batchEmployees.isEmpty()) {
                ...
                onboardingEmailLog.setDateErrorEmailsList("[]");
                onboardingEmailLogRepository.save(onboardingEmailLog);
                log.warn("No employees found for batch {}", employeeBatchLogId);
                return;
            }
            ...
            // 🟡 Process ONLY batch employees
            for (Employee employee : batchEmployees) {
                Boolean eligibility = isEligibleForOnboardingEmail(employee);
            ...
            log.info("Onboarding email sending completed for batch {}", employeeBatchLogId);
            log.info("Batch {} Results - Sent: {}, Failed: {}, Ineligible: {}, Date Errors: {}", 
                    employeeBatchLogId, emailsSent, emailsFailed, ineligibleDueToDate, dateFormatErrors);
    ...
    /**
     * Parse the batch_employee_emails field to extract email list
     */
    private List<String> parseBatchEmployeeEmails(String batchEmployeeEmails) {
        List<String> emails = new ArrayList<>();
        if (batchEmployeeEmails == null || batchEmployeeEmails.trim().isEmpty()) {
            return emails;
        }
        
        try {
            // Try to parse as JSON array first
            if (batchEmployeeEmails.trim().startsWith("[")) {
                String[] emailArray = objectMapper.readValue(batchEmployeeEmails, String[].class);
                emails = Arrays.asList(emailArray);
            } else {
                // Fallback: split by comma or newline
                String[] emailArray = batchEmployeeEmails.split("[,\n]");
            ...
            log.warn("Failed to parse batch employee emails as JSON, trying simple split: {}", e.getMessage());
        ...
        log.info("Parsed {} emails from batch employee emails: {}", emails.size(), emails);
        return emails;
    }
