...
public class ZingHRFetchService {
    ...
    @Value("${zinghr.mail.enabled:true}")
...
//    }
    ...
    protected void processDto(ZingEmployeeDTO dto) {
        ...
        if (!TEST_MODE) {
            ...
            log.warn("TEST_MODE → DB save skipped for {}", emp.getEmployeeId());
        }

        // Send email only for new employees inside 15 days
        if (MAIL_ENABLED && isNew && isEligible(emp.getJoinDate())) {
            String category = ZingEmployeeMapper.determineCategory(dto);
            sendWelcomeEmailByCategory(emp, category);
        } else if (!MAIL_ENABLED && isNew && isEligible(emp.getJoinDate())) {
            log.info("MAIL DISABLED → Would send mail to {}", emp.getEmail());
        }
    }
...
//    }
    ...
    private void sendWelcomeEmailByCategory(Employee emp, String category) {
        String templateName;
        switch (category) {
            case "INTERN":
            case "CONSULTANT":
                templateName = "onboarding-welcome-intern-consultant";
                break;
            default:
        ...
        try {
            ...
            log.info("{} email sent to {}", category, emp.getEmail());
        } catch (Exception ex) {
            log.error("Email failed for {}: {}", emp.getEmail(), ex.getMessage());
        }
    }
}
